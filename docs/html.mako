...
<%def name="show_module(module)">
    ...
    <dt id="${f.refname}">
        <code class="name">
            ...
            % if len(params) > 4 or len(example_str) > 70:
                <%
                    params = "".join(p + ',<br/>' for p in params)
                %>
                <div>${f.funcdef()} ${ident(f.name)} (</div>
                <div style="padding-left: 2em">${params}</div>
                <div>) ${return_type}</div>
            % else:
                <%
                    params = ", ".join(params)
                %>
                <div>${f.funcdef()} ${ident(f.name)} (${params}) ${return_type} </div>
            % endif
        </code>
    </dt>
    <dd>${show_desc(f)}</dd>
  ...
  <section>
    ...
    % for c in classes:
      ...
      <dt id="${c.refname}"><code class="name class">
          % if len(params) > 4 or len(", ".join(params)) > 50:
              <%
              params = "".join(p + ',<br/>' for p in params)
              %>
              <div>class ${ident(c.name)} (</div>
              <div style="padding-left: 2em">${params}</div>
              <div>)</div>
          % elif params:
