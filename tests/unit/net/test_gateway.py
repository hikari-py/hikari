...
async def test_init_produces_valid_url(event_loop):
    ...
    gw = gateway.GatewayConnection(
        host="wss://gateway.discord.gg:4949/", loop=event_loop, token="1234"
    )
...
async def test__receive_json_closes_connection_if_payload_was_not_a_dict(event_loop):
    ...
    await gw._receive_json()
    gw.ws.recv.assert_any_call()
    gw._do_reidentify.assert_awaited_once_with(
        code=MockGateway.TYPE_ERROR, reason="Expected JSON object."
    )
...
@non_zombified_test
async def test__receive_json_when_receiving_zlib_payloads_collects_before_decoding(
    event_loop
):
    gw = MockGateway(
        host="wss://gateway.discord.gg:4949/",
...
@non_zombified_test
async def test_heartbeat_if_not_acknowledged_in_time_closes_connection_with_resume(
    event_loop
):
    gw = MockGateway(
        host="wss://gateway.discord.gg:4949/",
...
async def test__send_resume(event_loop):
    ...
    gw._send_json.assert_called_with(
        {"op": 6, "d": {"token": "1234", "session_id": 1234321, "seq": 69_420}}
    )
...
async def test__send_identify_when_not_redacted_default_behaviour(event_loop):
    ...
    with asynctest.patch(
        "hikari.net.gateway.python_version", new=lambda: "python3"
    ), asynctest.patch(
        "hikari.net.gateway.library_version", new=lambda: "vx.y.z"
    ), asynctest.patch(
        "platform.system", new=lambda: "leenuks"
    ):
...
@non_zombified_test
async def test__process_events_does_not_update_seq_if_not_provided_from_payload(
    event_loop
):
    gw = MockGateway(
        host="wss://gateway.discord.gg:4949/",
...
async def test_request_guild_members(event_loop):
    ...
    gw._send_json.assert_called_with(
        {"op": 8, "d": {"guild_id": "1234", "query": "", "limit": 0}}
    )
