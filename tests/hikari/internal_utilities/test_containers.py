...
class TestObjectProxy:
    def test_ObjectProxy___getattr__(self):
        dop = containers.ObjectProxy({"foo": "bar"})
    ...
    def test_json_module_handles_as_expected_flat(self):
        ...
        d = json.loads('{"foo": "bar"}', object_hook=containers.ObjectProxy)
        assert d == {"foo": "bar"}
        assert d.foo == "bar"
    ...
    def test_json_module_handles_as_expected_nested_in_object(self):
        ...
        d = json.loads('{"foo": "bar", "baz": [{"id": 1}, {"id": 2}]}', object_hook=containers.ObjectProxy)
...
class TestDefaultImmutableMapping:
    def test_init(self):
        m = containers.DefaultImmutableMapping({"foo": "bar"}, 12)
