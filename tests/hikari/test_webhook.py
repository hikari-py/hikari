...
class TestWebhook:
    ...
    @pytest.fixture()
    def webhook_obj(self, mock_components):
    ...
    @pytest.mark.asyncio
    async def test_execute_without_optionals(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(messages.Message)
        mock_components.rest.execute_webhook.return_value = mock_webhook
        assert await webhook_obj.execute() is mock_webhook
        mock_components.rest.execute_webhook.assert_called_once_with(
            webhook=123123,
    ...
    @pytest.mark.asyncio
    async def test_execute_with_optionals(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(messages.Message)
    ...
    @pytest.mark.asyncio
    async def test_safe_execute_without_optionals(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(messages.Message)
        mock_components.rest.safe_webhook_execute = mock.AsyncMock(return_value=mock_webhook)
        assert await webhook_obj.safe_execute() is mock_webhook
        mock_components.rest.safe_webhook_execute.assert_called_once_with(
            webhook=123123,
            webhook_token="blah.blah.blah",
            content=...,
            username=...,
    ...
    @pytest.mark.asyncio
    async def test_safe_execute_with_optionals(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(messages.Message)
        mock_files = mock.MagicMock(files.BaseStream)
        mock_embed = mock.MagicMock(embeds.Embed)
        mock_components.rest.safe_webhook_execute = mock.AsyncMock(return_value=mock_webhook)
    ...
    @pytest.mark.asyncio
    async def test_delete_with_token(self, webhook_obj, mock_components):
        mock_components.rest.delete_webhook.return_value = ...
        assert await webhook_obj.delete() is None
        mock_components.rest.delete_webhook.assert_called_once_with(webhook=123123, webhook_token="blah.blah.blah")

    @pytest.mark.asyncio
    async def test_delete_without_token(self, webhook_obj, mock_components):
        webhook_obj.token = None
        mock_components.rest.delete_webhook.return_value = ...
        assert await webhook_obj.delete() is None
        mock_components.rest.delete_webhook.assert_called_once_with(webhook=123123, webhook_token=...)

    @pytest.mark.asyncio
    async def test_delete_with_use_token_set_to_true(self, webhook_obj, mock_components):
        mock_components.rest.delete_webhook.return_value = ...
        assert await webhook_obj.delete(use_token=True) is None
        mock_components.rest.delete_webhook.assert_called_once_with(webhook=123123, webhook_token="blah.blah.blah")

    @pytest.mark.asyncio
    async def test_delete_with_use_token_set_to_false(self, webhook_obj, mock_components):
        mock_components.rest.delete_webhook.return_value = ...
    ...
    @_helpers.assert_raises(type_=ValueError)
    @pytest.mark.asyncio
    async def test_delete_raises_value_error_when_use_token_set_to_true_without_token(
        self, webhook_obj, mock_components
    ):
        webhook_obj.token = None
        await webhook_obj.delete(use_token=True)

    @pytest.mark.asyncio
    async def test_edit_without_optionals_nor_token(self, webhook_obj, mock_components):
        webhook_obj.token = None
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.update_webhook.return_value = mock_webhook
        assert await webhook_obj.edit() is mock_webhook
        mock_components.rest.update_webhook.assert_called_once_with(
            webhook=123123, webhook_token=..., name=..., avatar=..., channel=..., reason=...
        )

    @pytest.mark.asyncio
    async def test_edit_with_optionals_and_token(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
    ...
    @pytest.mark.asyncio
    async def test_edit_with_use_token_set_to_true(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.update_webhook.return_value = mock_webhook
    ...
    @pytest.mark.asyncio
    async def test_edit_with_use_token_set_to_false(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.update_webhook.return_value = mock_webhook
    ...
    @_helpers.assert_raises(type_=ValueError)
    @pytest.mark.asyncio
    async def test_edit_raises_value_error_when_use_token_set_to_true_without_token(self, webhook_obj, mock_components):
        webhook_obj.token = None
        await webhook_obj.edit(use_token=True)

    @pytest.mark.asyncio
    async def test_fetch_channel(self, webhook_obj, mock_components):
        webhook_obj.channel_id = bases.Snowflake(202020)
        mock_channel = mock.MagicMock(channels.GuildChannel)
        mock_components.rest.fetch_channel.return_value = mock_channel
        assert await webhook_obj.fetch_channel() is mock_channel
        mock_components.rest.fetch_channel.assert_called_once_with(channel=202020)

    @pytest.mark.asyncio
    async def test_fetch_guild(self, webhook_obj, mock_components):
        webhook_obj.guild_id = bases.Snowflake(202020)
        mock_channel = mock.MagicMock(channels.GuildChannel)
        mock_components.rest.fetch_guild.return_value = mock_channel
        assert await webhook_obj.fetch_guild() is mock_channel
        mock_components.rest.fetch_guild.assert_called_once_with(guild=202020)

    @pytest.mark.asyncio
    async def test_fetch_self_with_token(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.fetch_webhook.return_value = mock_webhook
        assert await webhook_obj.fetch_self() is mock_webhook
        mock_components.rest.fetch_webhook.assert_called_once_with(webhook=123123, webhook_token="blah.blah.blah")

    @pytest.mark.asyncio
    async def test_fetch_self_without_token(self, webhook_obj, mock_components):
        webhook_obj.token = None
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.fetch_webhook.return_value = mock_webhook
        assert await webhook_obj.fetch_self() is mock_webhook
        mock_components.rest.fetch_webhook.assert_called_once_with(webhook=123123, webhook_token=...)

    @pytest.mark.asyncio
    async def test_fetch_self_with_use_token_set_to_true(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.fetch_webhook.return_value = mock_webhook
    ...
    @pytest.mark.asyncio
    async def test_fetch_self_with_use_token_set_to_false(self, webhook_obj, mock_components):
        mock_webhook = mock.MagicMock(webhooks.Webhook)
        mock_components.rest.fetch_webhook.return_value = mock_webhook
    ...
    @_helpers.assert_raises(type_=ValueError)
    @pytest.mark.asyncio
    async def test_fetch_self_raises_value_error_when_use_token_set_to_true_without_token(
        self, webhook_obj, mock_components
    ):
        webhook_obj.token = None
