...
class TestRESTReactionLogic:
    ...
    async def test_fetch_gateway_bot(self, rest_gateway_logic_impl):
        ...
        mock_gateway_bot_obj = mock.MagicMock(gateway_models.GatewayBot)
        rest_gateway_logic_impl._session.get_gateway_bot.return_value = mock_payload
        with mock.patch.object(gateway_models.GatewayBot, "deserialize", return_value=mock_gateway_bot_obj):
            ...
            gateway_models.GatewayBot.deserialize.assert_called_once_with(
                mock_payload, app=rest_gateway_logic_impl._app
            )
