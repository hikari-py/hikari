...
class TestInviteGuild:
    def test_deserialize(self, test_invite_guild_payload, mock_app):
        invite_guild_obj = invites.InviteGuild.deserialize(test_invite_guild_payload, app=mock_app)
...
class TestVanityUrl:
    ...
    def test_deserialize(self, vanity_url_payload, mock_app):
        vanity_url_obj = invites.VanityUrl.deserialize(vanity_url_payload, app=mock_app)
...
class TestInvite:
    def test_deserialize(
        ...
        with stack:
            invite_obj = invites.Invite.deserialize(test_invite_payload, app=mock_app)
            mock_target_user_deseralize.assert_called_once_with(test_2nd_user_payload, app=mock_app)
            mock_inviter_deseralize.assert_called_once_with(test_user_payload, app=mock_app)
            mock_channel_deseralize.assert_called_once_with(test_partial_channel, app=mock_app)
            mock_guld_deseralize.assert_called_once_with(test_invite_guild_payload, app=mock_app)
...
class TestInviteWithMetadata:
    def test_deserialize(self, test_invite_with_metadata_payload, mock_app):
        ...
        with stack:
            invite_with_metadata_obj = invites.InviteWithMetadata.deserialize(
                test_invite_with_metadata_payload, app=mock_app
            )
