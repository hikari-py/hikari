...
class TestUnicodeEmoji:
    ...
    def test_deserialize(self, mock_app):
        emoji_obj = emojis.UnicodeEmoji.deserialize({"name": "🤷"}, app=mock_app)
...
class TestCustomEmoji:
    def test_deserialize(self, mock_app):
        emoji_obj = emojis.CustomEmoji.deserialize({"id": "1234", "name": "test", "animated": True}, app=mock_app)
...
def test_deserialize_reaction_emoji_passes_kwargs(mock_app):
    emoji_obj = emojis.deserialize_reaction_emoji({"id": "1234", "name": "test"}, app=mock_app)
    assert emoji_obj._app is mock_app
