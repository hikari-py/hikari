...
class TestGuildMember:
    def test_deserialize(self, test_member_payload, test_user_payload, mock_app):
        ...
        with stack:
            guild_member_obj = guilds.GuildMember.deserialize(test_member_payload, app=mock_app)
            ...
            patched_user_deserializer.assert_called_once_with(test_user_payload, app=mock_app)
...
class TestActivityTimestamps:
    def test_deserialize(self, test_activity_timestamps_payload, mock_app):
        ...
        with stack:
            activity_timestamps_obj = guilds.ActivityTimestamps.deserialize(
                test_activity_timestamps_payload, app=mock_app
            )
...
class TestActivityAssets:
    def test_deserialize(self, test_activity_assets_payload, mock_app):
        activity_assets_obj = guilds.ActivityAssets.deserialize(test_activity_assets_payload, app=mock_app)
...
class TestActivitySecret:
    def test_deserialize(self, test_activity_secrets_payload, mock_app):
        activity_secret_obj = guilds.ActivitySecret.deserialize(test_activity_secrets_payload, app=mock_app)
...
class TestPresenceActivity:
    def test_deserialize(
        ...
        with stack:
            presence_activity_obj = guilds.PresenceActivity.deserialize(test_presence_activity_payload, app=mock_app)
        ...
        assert presence_activity_obj.timestamps._app is mock_app
...
class TestClientStatus:
    def test_deserialize(self, test_client_status_payload, mock_app):
        client_status_obj = guilds.ClientStatus.deserialize(test_client_status_payload, app=mock_app)
...
class TestPresenceUser:
    def test_deserialize_filled_presence_user(self, test_user_payload, mock_app):
        presence_user_obj = guilds.PresenceUser.deserialize(test_user_payload, app=mock_app)
    ...
    def test_deserialize_partial_presence_user(self, mock_app):
...
class TestGuildMemberPresence:
    def test_deserialize(
        ...
        assert guild_member_presence_obj.user._app is mock_app
        ...
        assert guild_member_presence_obj.activities[0]._app is mock_app
        assert guild_member_presence_obj.client_status == guilds.ClientStatus.deserialize(test_client_status_payload)
        assert guild_member_presence_obj.client_status._app is mock_app
...
class TestGuildMemberBan:
    ...
    def test_deserializer(self, test_guild_member_ban_payload, test_user_payload, mock_app):
        mock_user = mock.MagicMock(users.User)
        with _helpers.patch_marshal_attr(
            guilds.GuildMemberBan, "user", deserializer=users.User.deserialize, return_value=mock_user
        ) as patched_user_deserializer:
            guild_member_ban_obj = guilds.GuildMemberBan.deserialize(test_guild_member_ban_payload, app=mock_app)
            patched_user_deserializer.assert_called_once_with(test_user_payload, app=mock_app)
...
class TestIntegrationAccount:
    def test_deserializer(self, test_integration_account_payload, mock_app):
        integration_account_obj = guilds.IntegrationAccount.deserialize(test_integration_account_payload, app=mock_app)
...
class TestPartialGuildIntegration:
    def test_deserialise(self, test_partial_guild_integration_payload, test_integration_account_payload, mock_app):
        partial_guild_integration_obj = guilds.PartialGuildIntegration.deserialize(
            test_partial_guild_integration_payload, app=mock_app
        )
        ...
        assert partial_guild_integration_obj.account._app is mock_app
...
class TestGuildIntegration:
    ...
    def test_deserialize(
        self, test_guild_integration_payload, test_user_payload, test_integration_account_payload, mock_app
    ):
        mock_user = mock.MagicMock(users.User)
        ...
        with stack:
            guild_integration_obj = guilds.GuildIntegration.deserialize(test_guild_integration_payload, app=mock_app)
            patched_user_deserializer.assert_called_once_with(test_user_payload, app=mock_app)
...
class TestUnavailableGuild:
    def test_deserialize_when_unavailable_is_defined(self, mock_app):
        guild_delete_event_obj = guilds.UnavailableGuild.deserialize(
            {"id": "293293939", "unavailable": True}, app=mock_app
        )
        assert guild_delete_event_obj.is_unavailable is True

    def test_deserialize_when_unavailable_is_undefined(self, mock_app):
        guild_delete_event_obj = guilds.UnavailableGuild.deserialize({"id": "293293939"}, app=mock_app)
        assert guild_delete_event_obj.is_unavailable is True


class TestPartialGuild:
    def test_deserialize(self, test_partial_guild_payload, mock_app):
        partial_guild_obj = guilds.PartialGuild.deserialize(test_partial_guild_payload, app=mock_app)
...
class TestGuildPreview:
    def test_deserialize(self, test_guild_preview_payload, test_emoji_payload, mock_app):
        ...
        with mock.patch.object(emojis.KnownCustomEmoji, "deserialize", return_value=mock_emoji):
            guild_preview_obj = guilds.GuildPreview.deserialize(test_guild_preview_payload, app=mock_app)
...
class TestGuild:
    def test_deserialize(
        ...
        with stack:
            guild_obj = guilds.Guild.deserialize(test_guild_payload, app=mock_app)
        ...
        assert guild_obj.roles[41771983423143936]._app is mock_app
