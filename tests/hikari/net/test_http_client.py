...
class TestHTTPClient:
    @pytest.fixture
    def http_client_impl(self):
        class HTTPClientImpl(http_client.HTTPClient):
            def __init__(self, *args, **kwargs):
                self.base_url = "https://discordapp.com/api/v7"
    ...
    def test__init__with_bot_token_and_without_optionals(self, mock_init):
        ...
        with mock.patch.object(ratelimits, "ManualRateLimiter", return_value=mock_manual_rate_limiter):
            with mock.patch.object(
                ...
                client = http_client.HTTPClient(token="Bot token.otacon.a-token")
                base_http_client.BaseHTTPClient.__init__.assert_called_once_with(
    ...
    def test__init__with_optionals(self, mock_init):
        ...
        mock_connetor = mock.MagicMock(aiohttp.BaseConnector)
    ...
    async def test_close(self, http_client_impl):
        with mock.patch.object(base_http_client.BaseHTTPClient, "close") as mock_close:
            await http_client_impl.close()
    ...
    @pytest.mark.asyncio
    async def test_get_gateway(self, http_client_impl):
        http_client_impl._request.return_value = {"url": "discord.discord///"}
        ...
        with mock.patch.object(routes, "GATEWAY", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_gateway() == "discord.discord///"
            routes.GATEWAY.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_gateway_bot(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GATEWAY_BOT", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_gateway_bot() is mock_response
            routes.GATEWAY_BOT.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_audit_log_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_AUDIT_LOGS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_audit_log("2929292929") is mock_response
            routes.GUILD_AUDIT_LOGS.compile.assert_called_once_with(http_client_impl.GET, guild_id="2929292929")
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_get_guild_audit_log_with_optionals(self, http_client_impl):
        mock_response = {"webhooks": [], "users": []}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_AUDIT_LOGS)
        with mock.patch.object(routes, "GUILD_AUDIT_LOGS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.get_guild_audit_log(
    ...
    @pytest.mark.asyncio
    async def test_get_channel(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_channel("20202020020202") is mock_response
            routes.CHANNEL.compile.assert_called_once_with(http_client_impl.GET, channel_id="20202020020202")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_modify_channel_without_optionals(self, http_client_impl):
    ...
    async def test_modify_channel_with_optionals(self, http_client_impl):
        ...
        mock_route = mock.MagicMock(routes.CHANNEL)
        with mock.patch.object(routes, "CHANNEL", compile=mock.MagicMock(return_value=mock_route)):
            result = await http_client_impl.modify_channel(
                "6942069420",
                position=22,
                topic="HAHAHAHHAHAHA",
                nsfw=True,
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_close(self, http_client_impl):
        mock_route = mock.MagicMock(routes.CHANNEL)
        with mock.patch.object(routes, "CHANNEL", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_close_channel("939392929") is None
            routes.CHANNEL.compile.assert_called_once_with(http_client_impl.DELETE, channel_id="939392929")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_channel_messages_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_MESSAGES", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_channel_messages("9292929292") is mock_response
            routes.CHANNEL_MESSAGES.compile.assert_called_once_with(http_client_impl.GET, channel_id="9292929292")
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_get_channel_messages_with_optionals(self, http_client_impl):
        mock_response = [{"id": "29492", "content": "Kon'nichiwa"}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.CHANNEL_MESSAGES)
        with mock.patch.object(routes, "CHANNEL_MESSAGES", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.get_channel_messages(
    ...
    @pytest.mark.asyncio
    async def test_get_channel_message(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_MESSAGE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_channel_message("1111111111", "42424242") is mock_response
            routes.CHANNEL_MESSAGE.compile.assert_called_once_with(
                http_client_impl.GET, channel_id="1111111111", message_id="42424242"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_message_without_optionals(self, http_client_impl):
    ...
    @pytest.mark.asyncio
    async def test_create_reaction(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_REACTION", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.create_reaction("20202020", "8484848", "emoji:2929") is None
            routes.OWN_REACTION.compile.assert_called_once_with(
                http_client_impl.PUT, channel_id="20202020", message_id="8484848", emoji="emoji:2929"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_delete_own_reaction(self, http_client_impl):
        mock_route = mock.MagicMock(routes.OWN_REACTION)
        with mock.patch.object(routes, "OWN_REACTION", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_own_reaction("20202020", "8484848", "emoji:2929") is None
            routes.OWN_REACTION.compile.assert_called_once_with(
                http_client_impl.DELETE, channel_id="20202020", message_id="8484848", emoji="emoji:2929"
            )
        http_client_impl._request.assert_called_once_with(mock_route)
    ...
    @pytest.mark.asyncio
    async def test_get_reactions_without_optionals(self, http_client_impl):
        mock_response = [{"id": "42"}]
        http_client_impl._request.return_value = mock_response
        ...
        with mock.patch.object(routes, "REACTIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_reactions("29292929", "48484848", "emoji:42") is mock_response
    ...
    @pytest.mark.asyncio
    async def test_get_reactions_with_optionals(self, http_client_impl):
        mock_response = [{"id": "42"}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.REACTIONS)
        with mock.patch.object(routes, "REACTIONS", compile=mock.MagicMock(return_value=mock_route)):
    ...
    async def test_delete_all_reactions(self, http_client_impl):
        ...
        with mock.patch.object(routes, "ALL_REACTIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_all_reactions("44444", "999999") is None
            routes.ALL_REACTIONS.compile.assert_called_once_with(
                http_client_impl.DELETE, channel_id="44444", message_id="999999"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_edit_message_without_optionals(self, http_client_impl):
        mock_response = {"flags": 3, "content": "edited for the win."}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.CHANNEL_MESSAGE)
        with mock.patch.object(routes, "CHANNEL_MESSAGE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.edit_message("9292929", "484848") is mock_response
            routes.CHANNEL_MESSAGE.compile.assert_called_once_with(
                http_client_impl.PATCH, channel_id="9292929", message_id="484848"
            )
        http_client_impl._request.assert_called_once_with(mock_route, json_body={})

    @pytest.mark.asyncio
    async def test_edit_message_with_optionals(self, http_client_impl):
        mock_response = {"flags": 3, "content": "edited for the win."}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.CHANNEL_MESSAGE)
        with mock.patch.object(routes, "CHANNEL_MESSAGE", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.edit_message(
    ...
    @pytest.mark.asyncio
    async def test_delete_message(self, http_client_impl):
        mock_route = mock.MagicMock(routes.CHANNEL_MESSAGE)
        with mock.patch.object(routes, "CHANNEL_MESSAGE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_message("20202", "484848") is None
            routes.CHANNEL_MESSAGE.compile.assert_called_once_with(
                http_client_impl.DELETE, channel_id="20202", message_id="484848"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_bulk_delete_messages(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_MESSAGES_BULK_DELETE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.bulk_delete_messages("111", ["222", "333"]) is None
            routes.CHANNEL_MESSAGES_BULK_DELETE.compile.assert_called_once_with(http_client_impl.POST, channel_id="111")
        http_client_impl._request.assert_called_once_with(mock_route, json_body={"messages": ["222", "333"]})

    @pytest.mark.asyncio
    async def test_edit_channel_permissions_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_PERMISSIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.edit_channel_permissions("101010101010", "100101010") is None
    ...
    async def test_edit_channel_permissions_with_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_PERMISSIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.edit_channel_permissions(
    ...
    @pytest.mark.asyncio
    async def test_get_channel_invites(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_INVITES", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_channel_invites("999999999") is mock_response
            routes.CHANNEL_INVITES.compile.assert_called_once_with(http_client_impl.GET, channel_id="999999999")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_channel_invite_without_optionals(self, http_client_impl):
    ...
    @pytest.mark.asyncio
    async def test_delete_channel_permission(self, http_client_impl):
        mock_route = mock.MagicMock(routes.CHANNEL_PERMISSIONS)
        with mock.patch.object(routes, "CHANNEL_PERMISSIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_channel_permission("9292929", "74574747") is None
            routes.CHANNEL_PERMISSIONS.compile.assert_called_once_with(
                http_client_impl.DELETE, channel_id="9292929", overwrite_id="74574747"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_trigger_typing_indicator(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_TYPING", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.trigger_typing_indicator("11111111111") is None
            routes.CHANNEL_TYPING.compile.assert_called_once_with(http_client_impl.POST, channel_id="11111111111")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_pinned_messages(self, http_client_impl):
        ...
        with mock.patch.object(routes, "CHANNEL_PINS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_pinned_messages("393939") is mock_response
            routes.CHANNEL_PINS.compile.assert_called_once_with(http_client_impl.GET, channel_id="393939")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_add_pinned_channel_message(self, http_client_impl):
        mock_route = mock.MagicMock(routes.CHANNEL_PIN)
        with mock.patch.object(routes, "CHANNEL_PINS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.add_pinned_channel_message("292929", "48458484") is None
            routes.CHANNEL_PINS.compile.assert_called_once_with(
                http_client_impl.PUT, channel_id="292929", message_id="48458484"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_delete_pinned_channel_message(self, http_client_impl):
        mock_route = mock.MagicMock(routes.CHANNEL_PIN)
        with mock.patch.object(routes, "CHANNEL_PIN", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_pinned_channel_message("929292", "292929") is None
            routes.CHANNEL_PIN.compile.assert_called_once_with(
                http_client_impl.DELETE, channel_id="929292", message_id="292929"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_list_guild_emojis(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_EMOJIS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.list_guild_emojis("9929292") is mock_response
            routes.GUILD_EMOJIS.compile.assert_called_once_with(http_client_impl.GET, guild_id="9929292")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_emoji(self, http_client_impl):
        mock_response = {"id": "444", "name": "nekonyan"}
        http_client_impl._request.return_value = mock_response
        ...
        with mock.patch.object(routes, "GUILD_EMOJI", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_emoji("292929", "44848") is mock_response
            routes.GUILD_EMOJI.compile.assert_called_once_with(
                http_client_impl.GET, guild_id="292929", emoji_id="44848"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_emoji_without_optionals(self, http_client_impl):
    ...
    @pytest.mark.asyncio
    async def test_create_guild_emoji_with_optionals(self, http_client_impl):
        mock_response = {"id": "33", "name": "OwO"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_EMOJI)
        mock_image_data = "data:image/png;base64,iVBORw0KGgpibGFo"
    ...
    @pytest.mark.asyncio
    async def test_modify_guild_emoji_without_optionals(self, http_client_impl):
        mock_response = {"id": "20202", "name": "jeje"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_EMOJI)
        with mock.patch.object(routes, "GUILD_EMOJI", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_guild_emoji("292929", "3484848") is mock_response
            routes.GUILD_EMOJI.compile.assert_called_once_with(
    ...
    async def test_modify_guild_emoji_with_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_EMOJI", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.modify_guild_emoji("292929", "3484848", name="ok", roles=["222", "111"])
                is mock_response
            )
            routes.GUILD_EMOJI.compile.assert_called_once_with(
                http_client_impl.PATCH, guild_id="292929", emoji_id="3484848"
    ...
    @pytest.mark.asyncio
    async def test_delete_guild_emoji(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_EMOJI)
        with mock.patch.object(routes, "GUILD_EMOJI", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_guild_emoji("202", "4454") is None
            routes.GUILD_EMOJI.compile.assert_called_once_with(http_client_impl.DELETE, guild_id="202", emoji_id="4454")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILDS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.create_guild("GUILD TIME") is mock_response
            routes.GUILDS.compile.assert_called_once_with(http_client_impl.POST)
        http_client_impl._request.assert_called_once_with(mock_route, json_body={"name": "GUILD TIME"})

    @pytest.mark.asyncio
    async def test_create_guild_with_optionals(self, http_client_impl):
        mock_response = {"id": "99999", "name": "Guildith-Sama"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD)
        mock_image_data = "data:image/png;base64,iVBORw0KGgpibGFo"
    ...
    @pytest.mark.asyncio
    async def test_get_guild(self, http_client_impl):
        mock_response = {"id": "42", "name": "Hikari"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD)
        with mock.patch.object(routes, "GUILD", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild("3939393993939") is mock_response
            routes.GUILD.compile.assert_called_once_with(http_client_impl.GET, guild_id="3939393993939")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_modify_guild_without_optionals(self, http_client_impl):
        mock_response = {"id": "42", "name": "Hikari"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD)
        with mock.patch.object(routes, "GUILD", compile=mock.MagicMock(return_value=mock_route)):
    ...
    @pytest.mark.asyncio
    async def test_delete_guild(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD)
        with mock.patch.object(routes, "GUILD", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_guild("92847478") is None
            routes.GUILD.compile.assert_called_once_with(http_client_impl.DELETE, guild_id="92847478")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_channels(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_CHANNELS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_channels("393939393") is mock_response
            routes.GUILD_CHANNELS.compile.assert_called_once_with(http_client_impl.GET, guild_id="393939393")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_channel_without_optionals(self, http_client_impl):
    ...
    async def test_create_guild_channel_with_optionals(self, http_client_impl):
        ...
        mock_route = mock.MagicMock(routes.GUILD_CHANNELS)
        with mock.patch.object(routes, "GUILD_CHANNELS", compile=mock.MagicMock(return_value=mock_route)):
            result = await http_client_impl.create_guild_channel(
                "292929",
                "I am a channel",
                type_=2,
    ...
    @pytest.mark.asyncio
    async def test_modify_guild_channel_positions(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_CHANNELS)
        with mock.patch.object(routes, "GUILD_CHANNELS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.modify_guild_channel_positions("379953393319542784", ("29292", 0), ("3838", 1))
                is None
            )
            routes.GUILD_CHANNELS.compile.assert_called_once_with(http_client_impl.PATCH, guild_id="379953393319542784")
        http_client_impl._request.assert_called_once_with(
            mock_route, json_body=[{"id": "29292", "position": 0}, {"id": "3838", "position": 1}]
        )

    @pytest.mark.asyncio
    async def test_get_guild_member(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_MEMBER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_member("115590097100865541", "379953393319542784") is mock_response
            routes.GUILD_MEMBER.compile.assert_called_once_with(
                http_client_impl.GET, guild_id="115590097100865541", user_id="379953393319542784"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_list_guild_members_without_optionals(self, http_client_impl):
        mock_response = [{"id": "379953393319542784", "nick": "Big Moh"}]
        http_client_impl._request.return_value = mock_response
        ...
        with mock.patch.object(routes, "GUILD_MEMBERS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.list_guild_members("115590097100865541") is mock_response
            routes.GUILD_MEMBERS.compile.assert_called_once_with(http_client_impl.GET, guild_id="115590097100865541")
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_list_guild_members_with_optionals(self, http_client_impl):
        mock_response = [{"id": "379953393319542784", "nick": "Big Moh"}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_MEMBERS)
        with mock.patch.object(routes, "GUILD_MEMBERS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.list_guild_members("115590097100865541", limit=5, after="4444444444")
                is mock_response
            )
            routes.GUILD_MEMBERS.compile.assert_called_once_with(http_client_impl.GET, guild_id="115590097100865541")
        http_client_impl._request.assert_called_once_with(mock_route, query={"limit": 5, "after": "4444444444"})

    @pytest.mark.asyncio
    async def test_modify_guild_member_without_optionals(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER)
        with mock.patch.object(routes, "GUILD_MEMBER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_guild_member("115590097100865541", "379953393319542784") is None
            routes.GUILD_MEMBER.compile.assert_called_once_with(
    ...
    async def test_modify_guild_member_with_optionals(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER)
        with mock.patch.object(routes, "GUILD_MEMBER", compile=mock.MagicMock(return_value=mock_route)):
            result = await http_client_impl.modify_guild_member(
                "115590097100865541",
                "379953393319542784",
                nick="QT",
                roles=["222222222"],
    ...
    @pytest.mark.asyncio
    async def test_modify_current_user_nick_without_reason(self, http_client_impl):
    ...
    async def test_modify_current_user_nick_with_reason(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_GUILD_NICKNAME", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.modify_current_user_nick("202020202", "Nickname me", reason="Look at me") is None
            )
            routes.OWN_GUILD_NICKNAME.compile.assert_called_once_with(http_client_impl.PATCH, guild_id="202020202")
        http_client_impl._request.assert_called_once_with(
            mock_route, json_body={"nick": "Nickname me"}, reason="Look at me"
        )

    @pytest.mark.asyncio
    async def test_add_guild_member_role_without_reason(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_MEMBER_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.add_guild_member_role("3939393", "2838383", "84384848") is None
    ...
    async def test_add_guild_member_role_with_reason(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_MEMBER_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.add_guild_member_role(
        ...
        http_client_impl._request.assert_called_once_with(mock_route, reason="A special role for a special somebody")

    @pytest.mark.asyncio
    async def test_remove_guild_member_role_without_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER_ROLE)
        with mock.patch.object(routes, "GUILD_MEMBER_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.remove_guild_member_role("22222", "3333", "44444") is None
            routes.GUILD_MEMBER_ROLE.compile.assert_called_once_with(
    ...
    async def test_remove_guild_member_role_with_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER_ROLE)
        with mock.patch.object(routes, "GUILD_MEMBER_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.remove_guild_member_role("22222", "3333", "44444", reason="bye") is None
            routes.GUILD_MEMBER_ROLE.compile.assert_called_once_with(
                http_client_impl.DELETE, guild_id="22222", user_id="3333", role_id="44444"
            )
        http_client_impl._request.assert_called_once_with(mock_route, reason="bye")

    @pytest.mark.asyncio
    async def test_remove_guild_member_without_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER)
        with mock.patch.object(routes, "GUILD_MEMBER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.remove_guild_member("393939", "82828") is None
            routes.GUILD_MEMBER.compile.assert_called_once_with(
    ...
    async def test_remove_guild_member_with_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_MEMBER)
        with mock.patch.object(routes, "GUILD_MEMBER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.remove_guild_member("393939", "82828", reason="super bye") is None
            routes.GUILD_MEMBER.compile.assert_called_once_with(
                http_client_impl.DELETE, guild_id="393939", user_id="82828"
            )
        http_client_impl._request.assert_called_once_with(mock_route, reason="super bye")

    @pytest.mark.asyncio
    async def test_get_guild_bans(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_BANS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_bans("292929") is mock_response
            routes.GUILD_BANS.compile.assert_called_once_with(http_client_impl.GET, guild_id="292929")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_ban(self, http_client_impl):
        mock_response = {"id": "3939393"}
        http_client_impl._request.return_value = mock_response
        ...
        with mock.patch.object(routes, "GUILD_BAN", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_ban("92929", "44848") is mock_response
            routes.GUILD_BAN.compile.assert_called_once_with(http_client_impl.GET, guild_id="92929", user_id="44848")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_ban_without_optionals(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_BAN)
        with mock.patch.object(routes, "GUILD_BAN", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.create_guild_ban("222", "444") is None
            routes.GUILD_BAN.compile.assert_called_once_with(http_client_impl.PUT, guild_id="222", user_id="444")
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_create_guild_ban_with_optionals(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_BAN)
        with mock.patch.object(routes, "GUILD_BAN", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.create_guild_ban("222", "444", delete_message_days=5, reason="TRUE") is None
            routes.GUILD_BAN.compile.assert_called_once_with(http_client_impl.PUT, guild_id="222", user_id="444")
        http_client_impl._request.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_remove_guild_ban_without_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_BAN)
        with mock.patch.object(routes, "GUILD_BAN", compile=mock.MagicMock(return_value=mock_route)):
    ...
    async def test_remove_guild_ban_with_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_BAN)
        with mock.patch.object(routes, "GUILD_BAN", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.remove_guild_ban("494949", "3737", reason="LMFAO") is None
            routes.GUILD_BAN.compile.assert_called_once_with(http_client_impl.DELETE, guild_id="494949", user_id="3737")
        http_client_impl._request.assert_called_once_with(mock_route, reason="LMFAO")

    @pytest.mark.asyncio
    async def test_get_guild_roles(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_ROLES", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_roles("909") is mock_response
            routes.GUILD_ROLES.compile.assert_called_once_with(http_client_impl.GET, guild_id="909")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_role_without_optionals(self, http_client_impl):
        mock_response = {"id": "42"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_ROLES)
        with mock.patch.object(routes, "GUILD_ROLES", compile=mock.MagicMock(return_value=mock_route)):
    ...
    async def test_create_guild_role_with_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_ROLES", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.create_guild_role(
    ...
    @pytest.mark.asyncio
    async def test_modify_guild_role_positions(self, http_client_impl):
        mock_response = [{"id": "444", "position": 0}, {"id": "999", "position": 1}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_ROLES)
        with mock.patch.object(routes, "GUILD_ROLES", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_guild_role_positions("292929", ("444", 0), ("999", 1)) is mock_response
            routes.GUILD_ROLES.compile.assert_called_once_with(http_client_impl.PATCH, guild_id="292929")
        http_client_impl._request.assert_called_once_with(
            mock_route, json_body=[{"id": "444", "position": 0}, {"id": "999", "position": 1}]
        )

    @pytest.mark.asyncio
    async def test_modify_guild_role_with_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_guild_role("999999", "54234") is mock_response
    ...
    async def test_modify_guild_role_without_optionals(self, http_client_impl):
        ...
        mock_route = mock.MagicMock(routes.GUILD_ROLE)
        with mock.patch.object(routes, "GUILD_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            result = await http_client_impl.modify_guild_role(
                "999999",
                "54234",
                name="HAHA",
                permissions=42,
    ...
    @pytest.mark.asyncio
    async def test_delete_guild_role(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_ROLE)
        with mock.patch.object(routes, "GUILD_ROLE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_guild_role("29292", "4848") is None
            routes.GUILD_ROLE.compile.assert_called_once_with(http_client_impl.DELETE, guild_id="29292", role_id="4848")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_prune_count(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_PRUNE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_prune_count("29292", 14) == 7
            routes.GUILD_PRUNE.compile.assert_called_once_with(http_client_impl.GET, guild_id="29292")
        http_client_impl._request.assert_called_once_with(mock_route, query={"days": 14})

    @pytest.mark.asyncio
    @pytest.mark.parametrize("mock_response", ({"pruned": None}, {}))
    async def test_begin_guild_prune_without_optionals_returns_none(self, http_client_impl, mock_response):
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_PRUNE)
        with mock.patch.object(routes, "GUILD_PRUNE", compile=mock.MagicMock(return_value=mock_route)):
    ...
    async def test_begin_guild_prune_with_optionals(self, http_client_impl):
        http_client_impl._request.return_value = {"pruned": 32}
        mock_route = mock.MagicMock(routes.GUILD_PRUNE)
        with mock.patch.object(routes, "GUILD_PRUNE", compile=mock.MagicMock(return_value=mock_route)):
            assert (
    ...
    @pytest.mark.asyncio
    async def test_get_guild_voice_regions(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_VOICE_REGIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_voice_regions("2393939") is mock_response
            routes.GUILD_VOICE_REGIONS.compile.assert_called_once_with(http_client_impl.GET, guild_id="2393939")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_invites(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_INVITES", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_invites("9292929") is mock_response
            routes.GUILD_INVITES.compile.assert_called_once_with(http_client_impl.GET, guild_id="9292929")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_integrations(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_INTEGRATIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_integrations("537340989808050216") is mock_response
            routes.GUILD_INTEGRATIONS.compile.assert_called_once_with(
                http_client_impl.GET, guild_id="537340989808050216"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_guild_integration_without_reason(self, http_client_impl):
    ...
    @pytest.mark.asyncio
    async def test_create_guild_integration_with_reason(self, http_client_impl):
        mock_response = {"id": "22222"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.GUILD_INTEGRATIONS)
        with mock.patch.object(routes, "GUILD_INTEGRATIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.create_guild_integration("2222", "twitch", "443223", reason="NAH m8")
                is mock_response
            )
            routes.GUILD_INTEGRATIONS.compile.assert_called_once_with(http_client_impl.POST, guild_id="2222")
        http_client_impl._request.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_modify_guild_integration_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_INTEGRATION", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_guild_integration("292929", "747474") is None
    ...
    async def test_modify_guild_integration_with_optionals(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_INTEGRATION)
        with mock.patch.object(routes, "GUILD_INTEGRATION", compile=mock.MagicMock(return_value=mock_route)):
            result = await http_client_impl.modify_guild_integration(
                "292929",
                "747474",
                expire_behaviour=2,
    ...
    @pytest.mark.asyncio
    async def test_delete_guild_integration_without_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_INTEGRATION)
        with mock.patch.object(routes, "GUILD_INTEGRATION", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_guild_integration("23992", "7474") is None
            routes.GUILD_INTEGRATION.compile.assert_called_once_with(
    ...
    async def test_delete_guild_integration_with_reason(self, http_client_impl):
        mock_route = mock.MagicMock(routes.GUILD_INTEGRATION)
        with mock.patch.object(routes, "GUILD_INTEGRATION", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_guild_integration("23992", "7474", reason="HOT") is None
            routes.GUILD_INTEGRATION.compile.assert_called_once_with(
                http_client_impl.DELETE, guild_id="23992", integration_id="7474"
            )
        http_client_impl._request.assert_called_once_with(mock_route, reason="HOT")

    @pytest.mark.asyncio
    async def test_sync_guild_integration(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_INTEGRATION_SYNC", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.sync_guild_integration("3939439", "84884") is None
            routes.GUILD_INTEGRATION_SYNC.compile.assert_called_once_with(
                http_client_impl.POST, guild_id="3939439", integration_id="84884"
            )
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_embed(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_EMBED", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_embed("4949") is mock_response
            routes.GUILD_EMBED.compile.assert_called_once_with(http_client_impl.GET, guild_id="4949")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_modify_guild_embed_without_reason(self, http_client_impl):
    ...
    async def test_modify_guild_embed_with_reason(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_EMBED", compile=mock.MagicMock(return_value=mock_route)):
            embed_obj = {"channel_id": "222", "enabled": True}
            assert await http_client_impl.modify_guild_embed("393939", embed_obj, reason="OK") is mock_response
            routes.GUILD_EMBED.compile.assert_called_once_with(http_client_impl.PATCH, guild_id="393939")
        http_client_impl._request.assert_called_once_with(mock_route, json_body=embed_obj, reason="OK")

    @pytest.mark.asyncio
    async def test_get_guild_vanity_url(self, http_client_impl):
        ...
        with mock.patch.object(routes, "GUILD_VANITY_URL", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_vanity_url("399393") is mock_response
            routes.GUILD_VANITY_URL.compile.assert_called_once_with(http_client_impl.GET, guild_id="399393")
        http_client_impl._request.assert_called_once_with(mock_route)

    def test_get_guild_widget_image_url_without_style(self, http_client_impl):
        url = http_client_impl.get_guild_widget_image_url("54949")
        assert url == "https://discordapp.com/api/v7/guilds/54949/widget.png"

    def test_get_guild_widget_image_url_with_style(self, http_client_impl):
        url = http_client_impl.get_guild_widget_image_url("54949", style="banner2")
        assert url == "https://discordapp.com/api/v7/guilds/54949/widget.png?style=banner2"

    @pytest.mark.asyncio
    async def test_get_invite_without_counts(self, http_client_impl):
        ...
        with mock.patch.object(routes, "INVITE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_invite("fesdfes") is mock_response
            routes.INVITE.compile.assert_called_once_with(http_client_impl.GET, invite_code="fesdfes")
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_get_invite_with_counts(self, http_client_impl):
        mock_response = {"code": "fesdfes"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.INVITE)
        with mock.patch.object(routes, "INVITE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_invite("fesdfes", with_counts=True) is mock_response
            routes.INVITE.compile.assert_called_once_with(http_client_impl.GET, invite_code="fesdfes")
        http_client_impl._request.assert_called_once_with(mock_route, query={"with_counts": "True"})

    @pytest.mark.asyncio
    async def test_delete_invite(self, http_client_impl):
        mock_response = {"code": "diidsk"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.INVITE)
        with mock.patch.object(routes, "INVITE", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.delete_invite("diidsk") is mock_response
            routes.INVITE.compile.assert_called_once_with(http_client_impl.DELETE, invite_code="diidsk")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_current_application_info(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OAUTH2_APPLICATIONS_ME", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_current_application_info() is mock_response
            routes.OAUTH2_APPLICATIONS_ME.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_current_user(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_USER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_current_user() is mock_response
            routes.OWN_USER.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_user(self, http_client_impl):
        ...
        with mock.patch.object(routes, "USER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_user("54959") is mock_response
            routes.USER.compile.assert_called_once_with(http_client_impl.GET, user_id="54959")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_modify_current_user_without_optionals(self, http_client_impl):
        mock_response = {"id": "44444", "username": "Watashi"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.OWN_USER)
        with mock.patch.object(routes, "OWN_USER", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.modify_current_user() is mock_response
            routes.OWN_USER.compile.assert_called_once_with(http_client_impl.PATCH)
        http_client_impl._request.assert_called_once_with(mock_route, json_body={})

    @pytest.mark.asyncio
    async def test_modify_current_user_with_optionals(self, http_client_impl):
        mock_response = {"id": "44444", "username": "Watashi"}
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.OWN_USER)
        mock_image_data = "data:image/png;base64,iVBORw0KGgpibGFo"
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_connections(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_CONNECTIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_current_user_connections() is mock_response
            routes.OWN_CONNECTIONS.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_current_user_guilds_without_optionals(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_GUILDS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_current_user_guilds() is mock_response
            routes.OWN_GUILDS.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route, query={})

    @pytest.mark.asyncio
    async def test_get_current_user_guilds_with_optionals(self, http_client_impl):
        mock_response = [{"id": "452", "owner_id": "4949"}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.OWN_GUILDS)
        with mock.patch.object(routes, "OWN_GUILDS", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.get_current_user_guilds(before="292929", after="22288", limit=5) is mock_response
            )
            routes.OWN_GUILDS.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_leave_guild(self, http_client_impl):
        ...
        with mock.patch.object(routes, "LEAVE_GUILD", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.leave_guild("292929") is None
            routes.LEAVE_GUILD.compile.assert_called_once_with(http_client_impl.DELETE, guild_id="292929")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_dm(self, http_client_impl):
        ...
        with mock.patch.object(routes, "OWN_DMS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.create_dm("409491291156774923") is mock_response
            routes.OWN_DMS.compile.assert_called_once_with(http_client_impl.POST)
        http_client_impl._request.assert_called_once_with(mock_route, json_body={"recipient_id": "409491291156774923"})

    @pytest.mark.asyncio
    async def test_list_voice_regions(self, http_client_impl):
        ...
        with mock.patch.object(routes, "VOICE_REGIONS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.list_voice_regions() is mock_response
            routes.VOICE_REGIONS.compile.assert_called_once_with(http_client_impl.GET)
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_create_webhook_without_optionals(self, http_client_impl):
    ...
    @pytest.mark.asyncio
    async def test_get_channel_webhooks(self, http_client_impl):
        mock_response = [{"channel_id": "39393993", "id": "8383838"}]
        http_client_impl._request.return_value = mock_response
        mock_route = mock.MagicMock(routes.CHANNEL_WEBHOOKS)
        with mock.patch.object(routes, "CHANNEL_WEBHOOKS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_channel_webhooks("9393939") is mock_response
            routes.CHANNEL_WEBHOOKS.compile.assert_called_once_with(http_client_impl.GET, channel_id="9393939")
        http_client_impl._request.assert_called_once_with(mock_route)

    @pytest.mark.asyncio
    async def test_get_guild_webhooks(self, http_client_impl):
        mock_response = [{"channel_id": "39393993", "id": "8383838"}]
        http_client_impl._request.return_value = mock_response
        ...
        with mock.patch.object(routes, "GUILD_WEBHOOKS", compile=mock.MagicMock(return_value=mock_route)):
            assert await http_client_impl.get_guild_webhooks("9393939") is mock_response
            routes.GUILD_WEBHOOKS.compile.assert_called_once_with(http_client_impl.GET, guild_id="9393939")
        http_client_impl._request.assert_called_once_with(mock_route)
    ...
    async def test_modify_webhook_with_optionals_without_token(self, http_client_impl):
        ...
        with mock.patch.object(routes, "WEBHOOK", compile=mock.MagicMock(return_value=mock_route)):
            assert (
                await http_client_impl.modify_webhook(
