...
class IntentAwareEventDispatcherImpl(dispatchers.EventDispatcher):
    ...
    def add_listener(
        self, event_type: typing.Type[event_base.HikariEvent], callback: dispatchers.EventCallbackT, **kwargs
        ...
        Returns
        -------
        async def callback(event: hikari.events.base.HikariEvent) -> ...
        ...
        """
        if not issubclass(event_type, event_base.HikariEvent):
            raise TypeError("Events must subclass hikari.events.HikariEvent")

        required_intents = event_base.get_required_intents_for(event_type)
    ...
    def handle_exception(
        ...
        if not event_base.is_no_catch_event(event):
            self.logger.exception(
                'Exception occurred in handler for event "%s"', type(event).__name__, exc_info=exception
            )
            self.dispatch_event(other_events.ExceptionEvent(exception=exception, event=event, callback=callback))
        else:
            self.logger.exception(
