...
def raw_event_mapper(name: str) -> typing.Callable[[EventConsumerT], EventConsumerT]:
    ...
    def decorator(callable_: EventConsumerT) -> EventConsumerT:
        if not inspect.iscoroutinefunction(callable_):
            raise ValueError("Annotated element must be a coroutine function")

        event_set = getattr(callable_, EVENT_MARKER_ATTR, set())
        event_set.add(name)
        setattr(callable_, EVENT_MARKER_ATTR, event_set)
        return callable_
...
class EventManager(typing.Generic[EventDispatcherT], consumers.RawEventConsumer):
    ...
    async def process_raw_event(
        self, shard_client_obj: gateway_client.GatewayClient, name: str, payload: more_typing.JSONObject,
    ) -> None:
